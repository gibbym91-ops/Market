  ...
  try {
    const resp = await fetch("https://api.anthropic.com/v1/messages", {
      ...
      body: JSON.stringify({
        ...
        messages: [{
          role: "user",
          content: `You are a senior quantitative trading analyst. Research and analyze ${ticker} right now for a day trader.

The trader's strategy settings:
- Max loss tolerance: ${strategy.maxLoss}%
- Profit target: ${strategy.profitTarget}%
- Trading style: ${strategy.style}
- Position type preference: ${strategy.positionType}

Search for the latest news, price action, and market sentiment for ${ticker}. Then provide analysis.

Respond ONLY in JSON (no markdown, no backticks). Format:
{
  "current_price": "latest price you found or 'check chart'",
  ...
  "exit_target": "suggested exit/profit target",
  "stop_loss": "suggested stop loss level",
  "risk_reward": "estimated ratio like 1:2.5",
  "options_play": "suggested options strategy if applicable, or null",
  "catalysts": ["list of bullish factors found"],
  "risks": ["list of bearish risks found"],
  "key_levels": [{"price": "level", "label": "support/resistance description"}],
  "news_summary": ["brief summary of top 2-3 recent headlines found"],
  "intraday_bias": "description of expected intraday direction",
  "volume_analysis": "description of volume trends if found",
  "sector_context": "how the broader sector is performing"
}`
        }]
      })
    });
    ...
    setResult({
      summary: "Unable to reach AI analysis service. Please check your connection and try again.",
      action: "HOLD",
      confidence: "LOW",
      sentiment: "NEUTRAL",
      catalysts: [],
...
function SetupGuide({ alpacaConfig, setAlpacaConfig }) {
  return (
    <div style={{ padding: 20 }}>
      ...
      <p style={{ color: "#64748b", fontSize: 13, marginBottom: 20, lineHeight: 1.6 }}>
        Right now the chart and technical analysis run on TradingView's free widgets — totally real-time, no setup needed.
        When you're ready for deeper data (historical bars, options flow, automated trading), connect Alpaca below.
      </p>
      ...
      <div style={{
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 12 }}>
          ...
          <div>
            ...
            <div style={{ color: "#475569", fontSize: 11 }}>Free account • Real-time IEX data • Paper trading</div>
          </div>
          <div style={{
            marginLeft: "auto", padding: "4px 10px", borderRadius: 4, fontSize: 10,
            fontFamily: "var(--mono)", fontWeight: 700,
            background: alpacaConfig.apiKey ? "rgba(16,185,129,0.15)" : "rgba(245,158,11,0.15)",
            color: alpacaConfig.apiKey ? "#10b981" : "#f59e0b",
          }}>
            {alpacaConfig.apiKey ? "CONNECTED" : "NOT CONNECTED"}
          </div>
        </div>

        <div style={{ display: "grid", gap: 10 }}>
          <div>
            <label style={{ display: "block", color: "#64748b", fontSize: 10, fontFamily: "var(--mono)", letterSpacing: 1, marginBottom: 4 }}>
              API KEY ID
            </label>
            <input
              type="text"
              ...
              onChange={(e) => setAlpacaConfig(p => ({ ...p, apiKey: e.target.value }))}
              style={{
                width: "100%", padding: "8px 12px", borderRadius: 6,
                background: "#0a0f1e", border: "1px solid #1e293b",
                color: "#e2e8f0", fontSize: 13, fontFamily: "var(--mono)",
                outline: "none"
              }}
            />
          </div>
          <div>
            ...
            <input
              ...
              onChange={(e) => setAlpacaConfig(p => ({ ...p, apiSecret: e.target.value }))}
              style={{
                width: "100%", padding: "8px 12px", borderRadius: 6,
                background: "#0a0f1e", border: "1px solid #1e293b",
                color: "#e2e8f0", fontSize: 13, fontFamily: "var(--mono)",
          ...
          <div style={{ display: "flex", gap: 10 }}>
            <label style={{ display: "flex", alignItems: "center", gap: 6, cursor: "pointer" }}>
              <input
                type="radio" name="env" checked={alpacaConfig.paper}
                onChange={() => setAlpacaConfig(p => ({ ...p, paper: true }))}
              />
              <span style={{ color: "#94a3b8", fontSize: 12 }}>Paper Trading</span>
            </label>
            <label style={{ display: "flex", alignItems: "center", gap: 6, cursor: "pointer" }}>
              <input
                type="radio" name="env" checked={!alpacaConfig.paper}
                onChange={() => setAlpacaConfig(p => ({ ...p, paper: false }))}
              />
              <span style={{ color: "#94a3b8", fontSize: 12 }}>Live Trading</span>
            </label>
          </div>
        </div>
      </div>

      <div style={{ padding: 16, borderRadius: 10, border: "1px solid #1e293b55", background: "rgba(15,23,42,0.3)" }}>
        <div style={{ color: "#94a3b8", fontSize: 13, fontWeight: 600, marginBottom: 10 }}>Quick Start Guide</div>
        <div style={{ display: "grid", gap: 8 }}>
          {[
            { step: "1", text: "Go to alpaca.markets and create a free account", done: false },
            { step: "2", text: "Navigate to your dashboard → API Keys", done: false },
            { step: "3", text: "Generate a new API key pair", done: false },
            { step: "4", text: "Paste both keys above (start with Paper Trading!)", done: false },
            { step: "5", text: "The dashboard will begin pulling live Alpaca data", done: false },
          ].map(item => (
      ...
      <div style={{
        ...
        <div style={{ color: "#67e8f9", fontSize: 11, fontWeight: 600, marginBottom: 4 }}>💡 What's already working (no setup needed)</div>
        <div style={{ color: "#64748b", fontSize: 12, lineHeight: 1.5 }}>
          The TradingView chart, technical analysis widget, top movers, and news feed are all live right now with real market data.
          The AI Analysis tab uses Claude to search the web for real-time news and sentiment on any ticker. Alpaca adds historical data,
          options flow, and eventually automated execution.
        </div>
      </div>
    </div>
  );
    ...
    <div style={{
      ...
      <div style={{ display: "flex", height: "calc(100vh - 160px)" }}>
        ...
        <div style={{ flex: "1 1 0", minWidth: 0, overflow: "auto" }}>
          ...
          {activeTab === "setup" && (
            <SetupGuide alpacaConfig={alpacaConfig} setAlpacaConfig={setAlpacaConfig} />
          )}
        </div>
