...
Respond ONLY in JSON (no markdown, no backticks). Format:
{
  ...
  "sector_context": "how the broader sector is performing"
    ...
    // Extract text from response (handles web_search tool_use blocks mixed in)
    const text = data.content?.filter(i => i.type === "text").map(i => i.text).join("") || "";
    
    if (!text) {
      setResult({
        summary: "Claude returned an empty response. This can happen if the model used all its tokens on web search. Try again.",
        action: "HOLD", confidence: "LOW", sentiment: "NEUTRAL",
    ...
    setResult({
      summary: `Error: ${e.message}. This could be a timeout (analysis took too long), a JSON parsing issue, or a network problem. Try again.`,
      action: "HOLD",
      confidence: "LOW",
      sentiment: "NEUTRAL",
      catalysts: [],
